// scripts/import-albert-heijn-stores.mjs
// Fetches all Albert Heijn supermarket locations from OpenStreetMap and inserts
// them into the Supabase `assortments` table under business_id 353.
import { createClient } from "@supabase/supabase-js";
import "dotenv/config";

// ── Config ──────────────────────────────────────────────────────────────────
const BUSINESS_ID = 353;
const OVERPASS_URLS = [
...
(
  node["shop"="supermarket"]["name"~"Albert Heijn",i](49.4,2.3,53.6,7.2);
  way["shop"="supermarket"]["name"~"Albert Heijn",i](49.4,2.3,53.6,7.2);
);
out center body;
`;
...
const BORDER_FR = [
  [51.09, 2.54], [50.82, 2.60], [50.75, 2.88],
  [50.74, 3.10], [50.74, 3.22], [50.60, 3.35],
  [50.48, 3.55], [50.40, 3.70], [50.35, 3.85],
  [50.30, 4.05], [50.25, 4.20], [50.10, 4.35],
  [50.05, 4.50], [49.95, 4.80], [49.85, 5.00],
  [49.65, 5.25], [49.55, 5.50], [49.50, 5.82],
];

// DE/BE border — only relevant for Belgian Albert Heijn stores
const BORDER_DE_BE = [
...
function parseElement(el, sortOrder) {
  ...
  const storeName = city ? `Albert Heijn ${city}` : (tags.name || "Albert Heijn");
...
async function main() {
  ...
  for (const url of OVERPASS_URLS) {
    console.log(`Fetching Albert Heijn stores from ${url}...`);
    try {
